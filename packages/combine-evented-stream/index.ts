...
>(
  ...
  const proxies = collection.map(stream =>
    proxify(
      stream as EventTarget<'data' | 'error'>,
      ({ event, listener }) => {
        switch (event) {
          case 'data':
            return (chunk: StreamChunk) => listener(new ChunkCarrier<Stream, StreamChunk, StreamError>(chunk, stream))

          case 'error':
            return (error: StreamError) => listener(new ErrorCarrier<Stream, StreamError, StreamChunk>(error, stream))
        }
      },
    )
  )
  ...
  Promise.all(collection.map(
    target =>
      new Promise<void>(resolve =>
        target.addListener('close', function listener() {
  ...
  const createEvtMod = (modfn: EvtModFn): EvtMod =>
    (event: Event, fn: any) => {
      switch (event) {
        case 'close':
  ...
  class EvtStrCmb extends EventedStreamCombination<Stream, StreamChunk, StreamError> {
    public readonly addListener = createEvtMod((target, event, fn) => target.addListener(event, fn))

    public readonly removeListener = createEvtMod((target, event, fn) => target.removeListener(event, fn))
  }
